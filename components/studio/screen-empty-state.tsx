...
const DOTS_MASK = "radial-gradient(ellipse 70% 65% at 50% 50%, #000 0%, transparent 78%)";

/** Shared full-screen empty state. The preview shows representative generations from this app. */
export function ScreenEmptyState({ images, title, description, action, className }: ScreenEmptyStateProps) {
  return (
    <div className={cn("relative flex min-h-0 flex-1 items-center justify-center overflow-hidden px-6 py-12 text-center", className)}>
      <div
        aria-hidden
      ...
      <div className="relative flex w-full max-w-[444px] flex-col items-center gap-6">
        <div className="relative h-[105px] w-full" aria-hidden>
          <div className="absolute left-0 top-2.5 h-[85px] w-[38%] overflow-hidden rounded-2xl opacity-70">
            <img src={images[0]} alt="" className="size-full object-cover" />
            <span className="absolute inset-0 bg-black/30" />
          </div>
          <div className="absolute right-0 top-2.5 h-[85px] w-[38%] overflow-hidden rounded-2xl opacity-70">
