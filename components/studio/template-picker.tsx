...
import { BatteryFull, Calendar, House, WandSparkles, type LucideIcon } from "lucide-react";
...
const THUMBS = ["/presets/placeholder-1.svg", "/presets/placeholder-2.svg", "/presets/placeholder-3.svg", "/presets/placeholder-4.svg"] as const;
...
export const TEMPLATES: TemplateItem[] = [
  {
    ...
    icon: BatteryFull,
    prompt: "Cinematic product hero shot on a dark studio backdrop, slow orbit, soft rim light, dust particles catching the light.",
  },
  {
    id: "ugc-review",
    title: "UGC review",
    ...
    icon: WandSparkles,
    prompt: "Handheld creator-style review filmed on a phone in a sunny kitchen, natural light, casual framing, subtle camera shake.",
  },
  {
    id: "editorial-still",
    ...
    icon: Calendar,
    prompt: "Editorial fashion still, 85mm, shallow depth of field, muted palette, single hard light from the left.",
  },
  {
    id: "lifestyle-home",
    ...
    icon: House,
    prompt: "Warm lifestyle photo of a couple at home in the evening, lamp light, film grain, candid moment.",
  },
];
...
function GradientBadge({ as: Glyph, seed }: { as: LucideIcon; seed: string }) {
  return (
    <span className="relative flex size-9 shrink-0 items-center justify-center overflow-hidden rounded-[10px] border border-white/25 text-white shadow-[0_5px_3px_rgba(0,0,0,0.08),inset_0_3px_5px_rgba(255,255,255,0.24)]">
      <span aria-hidden className="absolute inset-0" style={{ backgroundImage: gradientFromSeed(seed) }} />
...
const TRIPTYCH = ["rounded-l-2xl rounded-r-sm", "rounded-sm", "rounded-r-2xl rounded-l-sm"] as const;
...
export function TemplateCard({ template, variant = "single", onTry, tryLabel = "Try" }: TemplateCardProps) {
    ...
    >
      <div className="flex h-60 items-stretch gap-1.5">
        {variant === "triptych" ? (
          template.images.map((src, i) => (
            <div key={i} className={cn("min-w-0 flex-1 overflow-hidden border border-white/10", TRIPTYCH[i])}>
              <img src={src} alt={`${template.title} — shot ${i + 1}`} className="size-full object-cover" />
            </div>
          ))
        ) : (
          <div className="min-w-0 flex-1 overflow-hidden rounded-2xl border border-white/10">
            <img src={template.images[0]} alt={template.title} className="size-full object-cover" />
          </div>
        )}
      </div>
      <div className="flex items-center gap-3 px-2 py-1">
        ...
        <div className="flex min-w-0 flex-1 flex-col gap-0.5">
          ...
          <span className="truncate text-xs text-muted-foreground">{template.subtitle}</span>
        </div>
        <Button
          size="sm"
...
export function ExamplePresets({ items, onUse, tryLabel = "Try", className = "w-full max-w-[900px]" }: ExamplePresetsProps) {
  return (
    <div className={cn("grid w-full grid-cols-1 gap-5 sm:grid-cols-2", className)}>
      {items.map((t) => (
        <TemplateCard key={t.id} template={t} onTry={onUse} tryLabel={tryLabel} />
      ))}
    </div>
  );
}
