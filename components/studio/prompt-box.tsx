...
import { AudioLines, Sparkle as SparklesGlyph } from "lucide-react"
...
function Root({
  ...
  return (
    <div
      className={cx("q-prompt-box", SURFACE_CLASS[surface], className)}
      {...props}
    >
      {modeRailChildren}
...
function Mode({
  ...
  const element = useRender({
    ...
    props: {
      className: cx(
        "q-prompt-box-mode",
        active && "q-prompt-box-mode-active",
        className
      ),
      ...
      children: (
        <>
          {start != null ? (
            <span className="q-prompt-box-mode-icon">{start}</span>
          ) : null}
          {children != null ? (
            <span className="q-prompt-box-mode-label">{children}</span>
          ) : null}
        </>
      ),
      ...props,
...
function Body({
  ...
  return (
    <div className={cx("q-prompt-box-body", className)} {...props}>
      <div className={cx("q-prompt-box-surface", surfaceClassName)}>
        {children}
      </div>
    </div>
  )
...
function Field({ className, rows = 1, ...props }: PromptBoxFieldProps) {
  return (
    <textarea
      rows={rows}
      className={cx("q-prompt-box-field", className)}
      {...props}
    />
  )
}
...
function Pill({
  ...
  const element = useRender({
    ...
    props: {
      className: cx(
        "q-prompt-box-pill",
        iconOnly && "q-prompt-box-pill-icon-only",
        className
      ),
      ...(render == null ? { type: "button" as const } : {}),
      children: (
        <>
          {start != null ? (
            <span className="q-prompt-box-pill-start">{start}</span>
          ) : null}
          {children != null ? (
            <span className="q-prompt-box-pill-label">{children}</span>
          ) : null}
          {end != null ? (
            <span className="q-prompt-box-pill-end">{end}</span>
          ) : null}
        </>
      ),
      ...props,
...
export type PromptBoxUploadProps = Omit<
  ComponentPropsWithRef<"button">,
  "children"
> & {
  ...
  src?: string
  /** String media presentation. Defaults to image. */
...
function Upload({
  ...
  const element = useRender({
    ...
    props: {
      className: cx(
        "q-prompt-box-upload",
        src != null && "q-prompt-box-upload-filled",
        className
      ),
      ...(render == null ? { type: "button" as const } : {}),
      children: (
        <>
          {src != null ? (
            mediaType === "audio" ? (
              <span
                className="flex size-full items-center justify-center bg-q-background-tertiary"
                role="img"
                aria-label={alt || "Audio reference"}
              >
                <AudioLines className="size-7 text-q-icon-secondary" />
              </span>
            ) : mediaType === "video" ? (
              <video
              ...
              <img className="q-prompt-box-upload-media" src={src} alt={alt} />
            )
          ) : null}
          <span className="q-prompt-box-upload-add">
            {add ?? <PlusGlyph />}
          </span>
          {label != null ? (
            <span className="q-prompt-box-upload-label">{label}</span>
          ) : null}
          {children}
        </>
      ),
        ...
        <>
          ...
          {hasMeta ? (
            <span className="q-prompt-box-generate-meta">
              {start ?? <SparklesGlyph fill="currentColor" />}
              {oldCost != null ? (
                <span className="q-prompt-box-generate-old">{oldCost}</span>
              ) : null}
              {cost != null ? (
                <span className="q-prompt-box-generate-cost">{cost}</span>
              ) : null}
            </span>
          ) : null}
        </>
