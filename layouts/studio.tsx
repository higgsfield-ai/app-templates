...
import { ExamplePresets, TEMPLATES, type TemplateItem } from "@/components/studio/template-picker";
import { HeroComposition } from "@/components/studio/hero-composition";
import { KeyDialog } from "@/components/studio/key-dialog";
import { MyProjects, type MyProjectsProject } from "@/components/studio/my-projects";
...
import type { GenerationPlane, MediaItem, MediaRole, Surface } from "@/generation/catalog";
import { useActive } from "@/generation/stores/active";
import { useSettings } from "@/generation/stores/settings";
import { runToGalleryItems, runToLibraryItems } from "@/lib/studio/gallery-items";
import { useProjects } from "@/lib/studio/projects";
import { uploadAsset, useUploads, type AssetSelection } from "@/lib/studio/uploads";
...
const HERO_FALLBACKS = ["/presets/placeholder-2.svg", "/presets/placeholder-1.svg", "/presets/placeholder-3.svg"] as const;

const HERO_GLOW = "radial-gradient(60% 80% at 50% 0%, rgba(160,164,170,0.14) 0%, rgba(160,164,170,0.05) 42%, transparent 72%)";
const HERO_DOTS = "radial-gradient(rgba(255,255,255,0.2) 1px, transparent 1px)";
const HERO_DOTS_MASK = "radial-gradient(55% 70% at 50% 0%, #000 0%, rgba(0,0,0,0.35) 45%, transparent 75%)";

type DockProps = Omit<ComponentProps<typeof StudioPromptBox>, "className" | "skin">;

function heroImages(items: GalleryItem[]): readonly [string, string, string] {
  const ready = items.filter((i) => i.status === "ready" && i.src !== "").slice(0, 3).map((i) => i.src);
  return [ready[0] ?? HERO_FALLBACKS[0], ready[1] ?? HERO_FALLBACKS[1], ready[2] ?? HERO_FALLBACKS[2]];
}

function HomeState({ title, items, projects, dock, onCreateProject, onOpenAll, onOpenProject, onUseTemplate }: {
  title: string;
  items: GalleryItem[];
  ...
  return (
    <div className="relative flex min-h-0 flex-1 flex-col items-center overflow-y-auto">
      <div aria-hidden className="pointer-events-none absolute inset-x-0 top-0 h-[600px]" style={{ backgroundImage: HERO_DOTS, backgroundSize: "14px 14px", maskImage: HERO_DOTS_MASK, WebkitMaskImage: HERO_DOTS_MASK }} />
      ...
      <div className="relative flex w-full flex-col items-center gap-12 px-6 pb-16 pt-16">
        ...
        <div className="flex w-full max-w-[900px] flex-col items-start gap-5">
          ...
          <div key={tab} className="w-full animate-in fade-in-0 duration-300">
            {tab === "projects" ? (
              <MyProjects projects={projects} generations={items} onCreateProject={onCreateProject} onOpenAllGenerations={onOpenAll} onOpenProject={onOpenProject} />
            ) : (
              <ExamplePresets
                items={TEMPLATES}
                onUse={(t) => {
...
function FeedState({ items, previewItems, title, dock, onDelete }: { items: GalleryItem[]; previewItems: GalleryItem[]; title: string; dock: DockProps; onDelete: (item: GalleryItem) => void }) {
    ...
    <div className="relative flex min-h-0 flex-1 flex-col overflow-hidden">
      <div className="flex min-h-0 flex-1 flex-col px-4 pb-40 pt-4">
        <UserGenerations
          ...
          emptyState={{
            images,
            title: title === "All Generations" ? "No generations yet" : `No generations in ${title}`,
            description: "Describe an idea below, then generate the first result.",
          }}
        />
      </div>
      <div className="pointer-events-none absolute inset-x-0 bottom-0 flex justify-center px-4 pb-4 pt-24" style={{ backgroundImage: "linear-gradient(to bottom, transparent 0%, var(--background) 100%)" }}>
        <StudioPromptBox {...dock} skin="glass" className="pointer-events-auto w-[900px] max-w-full" />
      </div>
    </div>
  );
}
...
export function StudioTemplate({ title = "Studio", headline = "Turn any idea into images and video" }: StudioTemplateProps) {
  ...
  const surfaces = useMemo(() => (["video", "image"] as Surface[]).filter((s) => MODELS.some((m) => m.surface === s)), []);
  ...
  const settings = settingsByModel[model.id] ?? {};

  useEffect(() => {
    void hasPlatformCredentials().then(setKeyConfigured);
  }, []);

  // Media roles differ per model; drop what the new model cannot take.
  useEffect(() => {
    setMedia((current) => current.filter((m) => (model.roles[m.role] ?? 0) > 0));
  }, [model]);

  const galleryItems = useMemo(() => runs.records.flatMap(runToGalleryItems), [runs.records]);
  const libraryItems = useMemo(() => [...uploads, ...runs.records.flatMap(runToLibraryItems)], [runs.records, uploads]);
  ...
  const projects = useMemo<MyProjectsProject[]>(
    () =>
      projectsStore.projects.map((p) => {
        ...
        const cover = items.find((i) => i.status === "ready" && i.src !== "")?.src;
        return { id: p.id, name: p.name, generationCount: new Set(items.map((i) => i.runId)).size, updatedAt: p.updatedAt, ...(cover ? { cover } : {}) };
      }),
    [byProject, projectsStore.projects],
  );
  const selectedProject = view.kind === "project" ? projects.find((p) => p.id === view.projectId) : undefined;
  const visibleItems = view.kind === "project" ? (byProject.get(view.projectId) ?? []) : galleryItems;

  const generating = runs.running.length > 0;
  const canGenerate = prompt.trim().length > 0 || media.length > 0;
  ...
  const handleGenerate = useCallback(() => {
    ...
    try {
      plane = { model: model.id, prompt: { text: prompt.trim() }, media: groupMedia(media, model.roles), settings: parseSettings(model, settings) };
    } catch (caught) {
    ...
    void runs.submit(plane, projectId).then((record) => {
      if (record && view.kind === "home") setView({ kind: "all" });
      if (record && projectId) projectsStore.touch(projectId);
    });
  }, [canGenerate, generating, keyConfigured, media, model, projectsStore, prompt, runs, settings, view]);

  const handleCancel = useCallback(() => {
    if (!generating || canceling) return;
    setCanceling(true);
    void Promise.all(runs.running.map((r) => runs.cancel(r.id))).finally(() => setCanceling(false));
  ...
  const dock: DockProps = {
    ...
    media,
    onMediaAdd: (role: MediaRole, selection: AssetSelection) => {
      setLocalError(null);
      setMedia((current) => [...current, { id: `${role}:${selection.id}`, url: selection.src, role }]);
    },
    onMediaRemove: (id) => setMedia((current) => current.filter((m) => m.id !== id)),
    ...
    error: localError ?? runs.error ?? undefined,
  };
  ...
  return (
    <div className="flex h-dvh overflow-hidden bg-background">
      <StudioSidebar
        ...
        projects={projects}
        onCreateProject={(name) => setView({ kind: "project", projectId: projectsStore.create(name).id })}
        onRenameProject={(id, name) => projectsStore.rename(id, name)}
        onDeleteProject={(id) => {
          projectsStore.remove(id);
          if (view.kind === "project" && view.projectId === id) setView({ kind: "all" });
        }}
        collapsed={collapsed}
      ...
      <main className="relative flex min-w-0 flex-1 flex-col">
        {view.kind === "home" ? (
          ...
          <FeedState items={visibleItems} previewItems={galleryItems} title={selectedProject?.name ?? "All Generations"} dock={dock} onDelete={(item) => runs.remove(item.runId)} />
        )}
      </main>
      <KeyDialog open={keyOpen} onOpenChange={setKeyOpen} configured={keyConfigured} onChange={setKeyConfigured} />
    </div>
  );
}

function groupMedia(items: MediaItem[], roles: Partial<Record<MediaRole, number>>): GenerationPlane["media"] {
  const media: GenerationPlane["media"] = {};
  for (const item of items) {
    const max = roles[item.role] ?? 0;
