import type { GenerationPlane, MediaRole, PlatformPaths, PlatformRequest } from "./types";
...
export function mapByPaths(plane: GenerationPlane, spec: PlatformPaths): PlatformRequest {
...
export function mapSeedance(plane: GenerationPlane, prefix: string): PlatformRequest {
  const start = urls(plane, "start")[0];
  const end = urls(plane, "end")[0];
  const refs = urls(plane, "reference");
  const videos = urls(plane, "video");
